...
volatile uint8_t serialbuff[7];
volatile uint8_t count10u;
volatile uint8_t RX_Timeout_ms;
...
/**
 ...
 INTERRUPT_HANDLER(UART1_RX_IRQHandler, 18)
 {
   ...
   if ((bufflen>=1) && (RX_Timeout_ms == 0)) bufflen = 0;
   if (bufflen<7)
   {
   ...
   RX_Timeout_ms = RS485_MESSAGE_TIMOUT_MS;
 }
...
/**
 ...
 INTERRUPT_HANDLER(TIM4_UPD_OVF_IRQHandler, 23)
 {
      ...
      if (count10u>=100)
    {
       if (RX_Timeout_ms>0) RX_Timeout_ms--;
       count10u = 0;
    }
